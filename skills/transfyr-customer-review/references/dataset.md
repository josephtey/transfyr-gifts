...
Load the customer dataset on the server after authentication. Keep actual customer evidence out of the public source repository. The current view uses schema version 6, with a private audit, grouped AI operations and separate human findings. The archived detailed view uses schema version 4.
...
type Session = {
  schemaVersion: 6; customer: string; operator: string;
