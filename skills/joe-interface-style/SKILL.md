...
- Keep results prominent when they frame the work, with concise metric labels and honest denominators. Avoid filling space with extra counters. For cohort comparisons, keep measured values separate from percentile positions: a simple stat above a LOW–HIGH performance scale, with percentile and share who performed better below. Avoid numeric axis labels or raw-value callouts that make two different percentage units look interchangeable. Use independent comparisons rather than reusing an overall score rank. Briefly identify an informal or uncontrolled cohort so the comparison is not overstated. If Joe accepts a visual estimate, round and label it instead of presenting an exact rank.
