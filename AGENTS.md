...
- The current review is an explanation of the reported result. Show the supplied outcome metrics, one coarse execution record per relevant calibration step, and only findings that contribute to the supplied explanation. No Major/Minor filters or labels. Keep all original AI actions, human review cells and excluded findings in the private audit. Do not relabel reported variability as a calculated CV or imply quantitative causal attribution.
- Dark mode, unbranded, video beside a single scrolling step-and-finding timeline. Use “System of Record” for neutral step summaries; human interpretation belongs in the findings. Draw restrained, animated connections from each step to its findings. Clicking a step or finding plays the corresponding interval. Follow playback at the protocol-step level. All steps start expanded; show counts of displayed findings and durations on the right. Manual expansion anchors to that step without seeking.
- The exact current AI action stays at the top of the video; hide it during annotation gaps. Findings and full descriptions stay below the scrubber without dismiss controls. Scrubber markers and chapters reflect only the explanation. Start paused at the first included error; explicit timestamps including zero override that default.
